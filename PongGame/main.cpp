...
public:
	...
	void Logic() {
		...
		//bottom wall
		if (bally == height - 1)
			ball->changeDiretion(ball->getDirection() == DOWNRIGHT ? UPRIGHT : UPLEFT);
			//top wall
		if (bally == 0)
			ball->changeDiretion(ball->getDirection() == UPRIGHT ? DOWNRIGHT : DOWNLEFT);
	//right wall
		if (ballx == width - 1)
			ScoreUp(player1);
		//left wall
